...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                nextState = [[[nextx, nexty], state[1], state[2], state[3], state[4]], direction, 1]
                for cornerIndex in range(0,4):
                    if (nextx, nexty) == self.corners[cornerIndex]:
...
def cornersHeuristic(state, problem):
    ...
    for index, corner in enumerate(corners):
        ...
        if not state[index + 1] and distance > maxDistance:
...
def foodHeuristic(state, problem):
    ...
    for x in range(0, foodGrid.width):
        for y in range(0, foodGrid.height):
            ...
            if foodGrid[x][y] and distance > maxDistance:
                maxDistance = distance
    return maxDistance
